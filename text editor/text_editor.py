...
def open_file():
    ...
    try:
        ...
        text_area.delete(1.0, END)
        file = open(file, "r")
        text_area.insert(file.read())
    except Exception:
        showerror("File Error", "Your file couldn´t be opened.")
    finally:
        file.close()

def save_file():
    file = asksaveasfilename(initialfile="untitled.txt", defaultextension=".txt",
                             file=[("All files", "*.*"), ("Text Document", "*.txt")])
    try:
        window.title(os.path.basename(file))
        file = open(file, "w")
        file.write(text_area.get(1.0, END))
    except Exception:
        showerror("Save Error", "Your file couldn´t be saved.")
    finally:
        file.close()
...
def about():
    showinfo("About the program", "This program was created using python")
